...
# -*- coding: utf-8 -*-
"""
Mask-only UPerNet detector.

This legacy detector predicts semantic masks together with a junction map
without vertex heatmap supervision.
"""
...
from topomapper.encoder import Encoder
...
class BuildingUPerNetDetector(nn.Module):
    def __init__(self, cfg, test=False):
        ...
        # Ver Head
        self.jloc_predictor = self._make_predictor(self.channels, 2)
    ...
    def forward_train(self, images, annotations = None):
        ...
        mask_pred, jloc_pred = self.predict(features)

        # Auxiliary branch outputs from FCNHead.
        mask_pred_aux, jloc_pred_aux = self.auxiliary_head(inputs)
    ...
    def predict(self, features):
        """Predict the segmentation mask and junction map."""
        # Feature heads
        ...
        # Ver Head
        jloc_pred = self.jloc_predictor(jloc_feature)  # (B, 2, H, W)
    ...
    def compute_loss(self, mask_pred, jloc_pred, mask_pred_aux, jloc_pred_aux, targets, loss_dict):
        ...
        # jloc_pred:  torch.Size([2, 2, 128, 128])
        # mask_pred_aux:  torch.Size([2, 7, 32, 32])
        # jloc_pred_aux:  torch.Size([2, 2, 32, 32])
        ...
        loss_dict['loss_mask'] = F.cross_entropy(mask_pred, targets['mask_ori'].squeeze(1).long())
        loss_dict['loss_jloc'] = self.junc_loss(jloc_pred, targets['jloc_ori'].squeeze(1).long())
        ...
        loss_dict['loss_mask_aux'] = F.cross_entropy(mask_pred_aux, targets['mask_ori'].squeeze(1).long())
        loss_dict['loss_jloc_aux'] = self.junc_loss(jloc_pred_aux, targets['jloc_ori'].squeeze(1).long())

        return loss_dict

    def compute_loss_128x128(self, mask_pred, jloc_pred, mask_pred_aux, jloc_pred_aux, targets, loss_dict):
        """Compute losses for the main and auxiliary branches at target resolution."""
        # mask_pred:  torch.Size([2, 7, 128, 128])
        # jloc_pred:  torch.Size([2, 2, 128, 128])
        # mask_pred_aux:  torch.Size([2, 7, 32, 32])
        # jloc_pred_aux:  torch.Size([2, 2, 32, 32])
        ...
        loss_dict['loss_jloc'] = self.junc_loss(jloc_pred, targets['jloc'].squeeze(1).long())

        loss_dict['loss_mask_aux'] = F.cross_entropy(mask_pred_aux, targets['mask_aux'].squeeze(1).long())
        loss_dict['loss_jloc_aux'] = self.junc_loss(jloc_pred_aux, targets['jloc_aux'].squeeze(1).long())
    ...
    def forward_test(self, images, annotations = None):
        ...
        # Upsample mask logits before softmax, and upsample the positive junction
        # probability map after softmax for smoother predictions.
        # Segmentation prediction.
        # Resize to the original image size before softmax.
        ...
        # Junction heatmap prediction.
        # Apply softmax first to preserve sharper responses, then upsample.
        jloc_prob = jloc_pred.softmax(dim=1)[:, 1:2]  # (B, 1, H/4, W/4)
        ...
        jloc_prob_up = F.interpolate(jloc_prob, size=(self.origin_height, self.origin_width),
                                     mode='bilinear', align_corners=False)
        jloc_prob_up = jloc_prob_up.squeeze(1)  # (B, H, W)
        """"""

        output = {
            'mask_prob': mask_prob,  # (B, num_classes, H, W), per-class probability map.
            'seg_mask': seg_mask,  # (B, H, W), int64 segmentation label map.
            'jloc_prob': jloc_prob_up,  # (B, H, W), float junction probability map.
