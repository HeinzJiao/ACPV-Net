...
class Backbone_VSSM(VSSM):
    def __init__(self,
                 out_indices=(0, 1, 2, 3),  # 指定哪些 stage 输出 feature map
                 pretrained=None,          # 预训练模型路径
                 norm_layer="ln",          # 每个输出层的归一化层类型
                 **kwargs):                # 其他传入 VSSM 的参数
        kwargs.update(norm_layer=norm_layer)
        super().__init__(**kwargs)
        self.channel_first = (norm_layer.lower() in ["bn", "ln2d"])  # 输出是否转成 [B,C,H,W]
        _NORMLAYERS = dict(
            ln=nn.LayerNorm,
            ln2d=LayerNorm2d,
            bn=nn.BatchNorm2d,
        ...
        for i in out_indices:
            # 为每一个输出层注册 norm 模块（outnorm0, outnorm1, ...）
            layer = norm_layer(self.dims[i])
            layer_name = f'outnorm{i}'
            self.add_module(layer_name, layer)

        # 删除原始的分类器（因为 segmentation 不需要）
        del self.classifier
    ...
    def forward(self, x):
        ...
        outs = []
        # 为每个指定输出层做了 norm，然后统一格式为 [B, C, H, W]，放入输出列表 outs 中
        ...
        return outs
        # outs = [out_stage0, out_stage1, out_stage2, out_stage3]
        # each: [B, C_i, H_i, W_i]
        # 0 torch.Size([B, 96, 128 (H/4), 128 (W/4)])
        # 1 torch.Size([B, 192, 64 (H/8), 64 (W/8)])
        # 2 torch.Size([B, 384, 32 (H/16), 32 (W/16)])
        # 3 torch.Size([B, 768, 16 (H/32), 16 (W/32)])
