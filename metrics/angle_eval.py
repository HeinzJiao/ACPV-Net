...
# Contour metrics:   0%|                                  | 0/220 [00:00<?, ?it/s]
# multiprocess.pool.RemoteTraceback:
# """
# Traceback (most recent call last):
#   File "/home/weiqin/anaconda3/envs/hisup/lib/python3.8/site-packages/multiprocess/pool.py", line 125, in worker
#     result = (True, func(*args, **kwds))
#   File "/home/weiqin/building_delineation/work4/utils/metrics/angle_eval.py", line 94, in compute_contour_metrics
...
#   File "/home/weiqin/building_delineation/work4/utils/metrics/angle_eval.py", line 121, in compute_polygon_contour_measures
...
# """
# 若出现如上报错，原因是 Shapely 2.x 的行为变化：在 Shapely 1.x 里 GeometryCollection 可以直接被 for 迭代；2.x 开始不再可迭代，需要显式用 .geoms 属性。
# 修复：把所有对 GeometryCollection 的迭代换成 .geoms。另外顺手加一个 area>0 的保护，避免极小或退化面导致被 0 除。
...
def compute_polygon_contour_measures(pred_polygons: list, gt_polygons: list, sampling_spacing: float, min_precision: float, max_stretch: float, metric_name: str="cosine", progressbar=False):
    ...
    # filtered_pred_polygons = [pred_polygon for pred_polygon in pred_polygons if min_precision < pred_polygon.intersection(gt_polygons).area / pred_polygon.area]
    # 1) 过滤预测多边形（注意 .geoms，并防 0 面积）
    filtered_pred_polygons = [
        p for p in pred_polygons.geoms
        if (p.area > 0) and (p.intersection(gt_polygons).area / p.area > min_precision)
    ]
