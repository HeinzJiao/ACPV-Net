...
# -*- coding: utf-8 -*-
"""
Extract vertex coordinates from predicted heatmaps.

This script applies NMS and score thresholding to `.npy` heatmaps, saves the
resulting vertex coordinates as JSON files, and optionally reports vertex-level
precision and recall against ground-truth annotations.

Example:
    python tools/extract_vertices_from_heatmap.py \
        --input_dir /path/to/heatmaps \
        --save_dir /path/to/vertices_json \
        --threshold 0.1 \
        --topk 300
"""
...
def extract_vertices(prob_map: np.ndarray, threshold: float, topk: int, kernel_size: int):
    ...
    if prob_map.ndim == 3 and prob_map.shape[0] == 3:
        ...
        prob_map = (prob_map + 1.0) / 2.0  # Normalize from [-1, 1] to [0, 1].
